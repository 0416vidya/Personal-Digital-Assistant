    ...
    try:
        ...
        if (body == '' and filename == ''):
            ...
            try:
                ...
                notification("Mail", "Mail sent successfully!!!", '/home/vidya/Downloads/mail.png')
                w1.withdraw()
...
def read():
    ...
    with sr.Microphone() as source:
        notification("Recognition", "listening...", '/home/vidya/Downloads/mic.png')
        r.adjust_for_ambient_noise(source)
        audio  = r.listen(source)
    input = ''
    notification("Recognition", "processing...", '/home/vidya/Downloads/mic.png')
    try:
...
def check():
    try:
        ...
        if not messages:
            ...
            notification("Mail", 'From: ' + email_from + '\n' + 'Subject: ' + email_subject + '\n' + 'Snippet: ' + email_snippet, '/home/vidya/Downloads/mail.png')
...
def remind(input2, time, ampm):
    ...
    if (len(time1[0]) < 3 and len(time1[1]) < 3 and int(time1[0]) < 24 and int(time1[0]) < 60 and int(time1[0]) > -1 and int(time1[1]) > -1):
        ...
        job1 = cron.new(command='/usr/bin/python3 notification.py',  comment=cmt)

        job1.minute.also.on(minutes)
        job1.hour.also.on(hours)

        cron.write()
        notification("Reminder", "Will remind you at " + time + ' ' + ampm, '/home/vidya/Downloads/reminder.png')
...
def fetch1(entries):
    ...
    if (length == 0):
        ...
        for i in range(1, length):
            input = input + input1[i]
        input = 'https://www.google.co.in/#q=' + input
        browse(input)
        ...
        notification("Time", time1, '/home/vidya/Downloads/time.png')
        speak(time1)
    elif (input1[0] == 'date' and length == 1):
        now = datetime.datetime.now()
        time1 = str(now.strftime("%Y-%m-%d %H:%M"))
        notification("Date", time1, '/home/vidya/Downloads/date.png')
        #speak(time1)
    else:
        try:
...
def makeform1(w):
    ...
    entries = []
    row = Frame(w)
    lab = Label(row, width=40, text='Hey I am John, How can I help you?', anchor='w')
    row.pack(side=TOP, fill=X, padx=5, pady=5)
    lab.pack()
    row = Frame(w)
    entry2 = Entry(row)
